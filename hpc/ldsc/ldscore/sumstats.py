...
def _print_jk_blocks(sumstats, separators, ofh, log):
    '''Prints an assignment of snps to jackknife blocks'''
    log.log('Printing jackknife block snp assignments {F}.'.format(F=ofh))
    blocks = np.zeros(sumstats.shape[0])
    for i in range(len(separators) - 1):
        blocks[separators[i]:separators[i+1]] = i
    sumstats[['SNP']].assign(block = blocks).to_csv(ofh, sep='\t', index=False)
...
def _read_ld_sumstats(args, log, fh, alleles=False, dropna=True):
    ...
    if args.set_jk_seps_wo_filtering:
        block_assignment = w_ld.assign(block = np.floor(np.arange(w_ld.shape[0]) * 200 / w_ld.shape[0]).astype(int))
        block_assignment = np.array(sumstats[['SNP']].merge(block_assignment).block)
        #w_ld.assign(block = np.floor(200 / np.arange(w_ld.shape[0])))
        #seps = np.floor(np.linspace(0, w_ld.shape[0], 201)).astype(int)
        #block_assignments = np.zeros(sumstats[w_ld_cname].shape)
        #current_block = -1
        #for i in range(len(block_assignments)):
        #    if i in seps:
        #        current_block += 1
        #    block_assignments[i] = current_block
        return M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols, block_assignment
    else:
...
def estimate_h2(args, log):
    ...
    if args.no_intercept:
        args.intercept_h2 = 1
    if args.set_jk_seps_wo_filtering:
        M_annot, w_ld_cname, ref_ld_cnames, sumstats, novar_cols, block_assignments = _read_ld_sumstats(
            args, log, args.h2)
    else:
    ...
    if chisq_max is not None:
        ...
        sumstats = sumstats.ix[ii, :]
        if args.set_jk_seps_wo_filtering:
            block_assignments = block_assignments[np.nonzero(ii)]
        log.log('Removed {M} SNPs with chi^2 > {C} ({N} SNPs remain)'.format(
                C=chisq_max, N=np.sum(ii), M=n_snp-np.sum(ii)))
        n_snp = np.sum(ii)  # lambdas are late-binding, so this works
        ref_ld = np.array(sumstats[ref_ld_cnames])
    ...
    if args.set_jk_seps_wo_filtering:
        separators = np.nonzero([i != j for (i, j) in zip(block_assignments[1:], block_assignments[:-1])])[0]
        separators = np.concatenate([[0], separators, [len(chisq)]])

    hsqhat = reg.Hsq(chisq, ref_ld, s(sumstats[w_ld_cname]), s(sumstats.N),
                     M_annot, n_blocks=n_blocks, intercept=args.intercept_h2,
                     twostep=args.two_step, old_weights=old_weights, separators = separators)
    # relevant arguments are probably hsqhat.jknife.separators
    # and some metadata with length 1185468
    ...
    if args.print_delete_vals:
        ...
        _print_jk_blocks(sumstats, hsqhat.jknife.separators, args.out + '.blocks', log)
